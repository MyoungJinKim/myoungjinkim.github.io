...
모터를 조종하기 위해서는 속도와 방향을 조절할 수 있어야 합니다. 우선 속도를 조절하는 방법을 알아보도록 하겠습니다. DC모터는 일반적으로 전압을 낮추면 천천히 돌고 전원을 높으면 빠르게 돌게 됩니다. 따라서 모터에 주는 전원을 직접 조정하여 모터의 속도를 조절할 수 있지만, 이 방법은 매우 비효율적입니다. 때문에 DC모터의 속도를 조절하는 데에는 주로 PWM (Pulse Width Modulation)이라는 방식을 사용합니다.
